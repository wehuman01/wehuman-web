...
// A sparse field of glyphs that mutate slowly on their own. Around the
// pointer they wake up: the signal ink, a little larger, swapping faster.
// Under prefers-reduced-motion or data-saver the field is drawn once,
// static, and only the pointer highlight updates.
...
(() => {
  ...
  const DENSITY = 0.3;    // share of slots that carry a glyph
  ...
  function readTheme() {
    ...
    baseCol = cs.getPropertyValue('--color-ink').trim();
